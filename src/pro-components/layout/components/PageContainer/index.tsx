...
import { Affix, Breadcrumb, ConfigProvider, SpinProps, Tabs, version } from "antd";
    ...
    <div className={`${prefixedClassName}-detail ${hashId}`.trim()}>
      <div className={`${prefixedClassName}-main ${hashId}`.trim()}>
        <div className={`${prefixedClassName}-row ${hashId}`.trim()}>
          {content && (
            <div className={`${prefixedClassName}-content ${hashId}`.trim()}>{content}</div>
          )}
          {extraContent && (
            <div className={`${prefixedClassName}-extraContent ${hashId}`.trim()}>
              {extraContent}
            </div>
          )}
        </div>
...
const PageContainerBase: React.FC<PageContainerProps> = (props) => {
  ...
  const content = useMemo(() => {
    return children ? (
      <>
        <div
          className={classNames(hashId, `${basePageContainer}-children-container`, {
      ...
      <>
        <div style={style} className={containerClassName}>
          {fixedHeader && pageHeaderDom ? (
            ...
            >
              <div className={`${basePageContainer}-warp ${hashId}`.trim()}>{pageHeaderDom}</div>
            </Affix>
          ) : (
            pageHeaderDom
          )}
